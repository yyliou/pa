...
> **Privacy rule.** The repository is public (GitHub Pages). The directories `stu/`, `pa-ex/`, `pa-mid/`, and `pa-fin/` contain student personal data (roster, submissions, per-question scores) and are therefore excluded from version control via `.gitignore`, together with all knitr `*_cache/` directories. They exist only in the local (pCloud-synced) working copy and must never be committed or pushed.
...
| Setting | Value |
|:---|:------|
| output-dir | .. (renders from web/ directly into the repository root for GitHub Pages; no manual copying) |
...
The header additionally includes an `include-after-body` script and title-slide attributes that inject a language-switch link (`chN-zh.html` in the English file; `chN.html` in the Chinese file). In the Chinese file the title, subtitle, author, and institute fields are localized (`author: "劉昱佑"`, `institute: "實踐大學"`).
...
- **Depth tiering:** Parts I–IV (Ch. 1–15) receive exhaustive coverage; Part V (Ch. 16–23, compressed into four teaching weeks) and Part VI (Ch. 24–26, self-study) may be condensed to core concepts with runnable examples (see prompt.Rmd, Instruction 2).
...
Step 3. Render both language versions (prompt/render.txt has the
        batch loop for all chapters):
...
4. Render (batch loop in prompt/render.txt):
   for f in ch*/ch*.qmd; do quarto render "$f"; done
...
for f in ch*/ch*.qmd; do quarto render "$f"; done
# Website pages render from web/ straight into the repo root:
