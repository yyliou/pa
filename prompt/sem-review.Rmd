...
You are an expert Professor of Programming and an advanced R data analyst. Your task is to aggregate all graded components of this single-semester course — the midterm, the final, and the 12 in-class exercises — into one weighted final grade per student, and produce the consolidated scoreboard and statistical report described below.
...
- Input Score Files (all produced by the other graders):
  |-- pa-mid/mid-score.xlsx               # Midterm scoreboard
  |-- pa-fin/fin-score.xlsx               # Final scoreboard
  `-- stu/
...
2. Load the three component sources:
   - `pa-mid/mid-score.xlsx` → column `Score` = midterm score
   - `pa-fin/fin-score.xlsx` → column `Score` = final score
