...
面板（lattice 的條件）在這裡叫**分面（facet）**，以公式指定：
...
按組上色只是另一種映射——同一份資料、單一面板：

```{r,echo=TRUE}
qplot(x=a, y=b, data=d, color=c)       # 用顏色取代面板
```

## 單變量：直方圖與密度圖 {.tight .fit-a}
...
```{r,echo=TRUE,fig.height=5,cache=TRUE}
temp <- tempfile(fileext = ".rda")
...
| `stat`／`position` | 統計變換／位置調整（書中年代——ggplot2 2.0 已移除） | `list(NULL)` |
...
- 光禿禿的 `ggplot(...)` 沒有圖層——書中年代直接列印會報錯（"No layers in plot"）；現代 ggplot2 只會畫出空面板。
- 書中年代的 `layer("point")` 以 geom 簡名加圖層；現代 `layer()` 要求明確給 `geom`、`stat`、`position`——實務上都用 `geom_*` 便利函式。
