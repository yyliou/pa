...
## A Simple Example: Formula and Conditioning {.tight}

Lattice arguments are consistent: a **formula** first (positional — its name has changed before!), then `data`. The same toy data, plain and then conditioned:
...
## A Simple Example: Groups {.tight}

**Grouping** keeps a single panel but distinguishes the groups by color/symbol — the third way to show the same data:
...
## Iterating to the Right Display {.tight .fit-a}
...
## qqmath on Prices: Finding the Log-Normal {.tight}
...
The log transform straightens the line nicely.

## qqmath on Prices: Split by Bedrooms {.tight}

Now split by bedrooms — groups, smooth lines instead of points (`type="smooth"`, an argument that flows to the panel function), explicit subset to keep the key clean:
